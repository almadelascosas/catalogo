                    ...
                    if ($productosCart->num_rows() > 0) {
                        foreach ($_SESSION['cart'] as $key2 => $value2) {
                            ...
                            foreach ($productosCart->result_array() as $key => $value) {
                                if ($value['productos_id']==$value2['producto_id']) {
                                    $agg=0;
                                    for ($i=0; $i < count($productosMostrar); $i++) {
                                        if (isset($productosMostrar[$i]['productos_id'])) {
                                            if($productosMostrar[$i]['productos_id']==$value2['producto_id']){
                                                if (floatval($productosMostrar[$i]['productos_precio'])==floatval($value2['precio'])) {
                                                    if ($agregado[$key2]!=1) {
                                                        $productosMostrar[$i]['productos_cantidad']=floatval($productosMostrar[$i]['productos_cantidad'])+floatval($value2['cantidad']);
                                                        $agregado[$key2] = 1;
                                                    }
                                                }
                                            }
                                    ...
                                    if ($agregado[$key2]!=1) {
                                        ...
                                        if ($value['productos_imagen_destacada']!="" && $value['productos_imagen_destacada']!=0) {
                                            $image=base_url().$value['medios_url'];
                                        }
                                        array_push($productosMostrar, array(
                                            ...
                                            'productos_precio' => floatval($value2['precio']),
                                            'productos_cantidad' => floatval($value2['cantidad']),
                    ...
                    <form onsubmit="loadedbutton('#btn-relizar-pedido');" action="<?=base_url()?>checkout/finalizar-compra" method="post" class="col-md-10 form-checkout offset-md-1 col-12">
                        <div class="row">
                            <div class="col-md-8 col-12">
                                ...
                                <?php
                                if (isset($_SESSION['municipio_session']) && isset($_SESSION['departamento_session']) && ($_SESSION['municipio_session']!=0) ) { ?>
                                <input type="hidden" name="pedidos_localidad" id="pedidos_localidad" value="<?=$_SESSION['municipio_session']?>">
                                <input type="hidden" name="pedidos_departamento" id="pedidos_departamento" value="<?=$_SESSION['departamento_session']?>">
                                <div class="form-group">
                                    <label for="pedidos_localidad">Localidad / Ciudad <sup style="color:red;">*</sup></label>
                                    <input type="text" disabled class="form-control" value="<?=$_SESSION['municipio_nombre']?>">
                                </div>
                                <div class="form-group">
                                    <label for="pedidos_departamento">Departamento <sup style="color:red;">*</sup></label>
                                    <input type="text" disabled class="form-control" value="<?=$_SESSION['departamento_nombre']?>">
                                </div>
                                <?php
                                }else{ ?>
                                <div class="form-group">
                                ...
                                <div class="row">
                                    ...
                                    <div id="div_otra_direccion" class="d-none col-12">
                                        ...
                                        <?php
                                        if (isset($_SESSION['municipio_session']) && isset($_SESSION['departamento_session']) && $_SESSION['municipio_session']!=0 ) { ?>
                                        <input type="hidden" name="pedidos_localidad_envio" id="pedidos_localidad_envio" value="<?=$_SESSION['municipio_session']?>">
                                        <input type="hidden" name="pedidos_departamento_envio" id="pedidos_departamento_envio" value="<?=$_SESSION['departamento_session']?>">
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label for="pedidos_departamento">Departamento <sup style="color:red;">*</sup></label>
                                            <select onchange="obtenerMunicipios(this,'#pedidos_localidad_envio')" class="form-control" name="pedidos_departamento_envio" id="pedidos_departamento_envio">
                                                <option value="">Seleccione...</option>
                                                <?php
                                        ...
                                        <div class="form-group">
                                            <label for="pedidos_localidad_envio">Localidad / Ciudad <sup style="color:red;">*</sup></label>
                                            <select class="form-control" name="pedidos_localidad_envio" id="pedidos_localidad_envio">
                                                <option value="">Seleccione...</option>
                                            </select>
                                        </div>
                                        <?php
                            ...
                            <div class="col-md-4 col-12 px-md-0">
                                <div class="card card-resumen-cart">
                                    <div class="col-12">
                                        <div class="row">
                                            <div class="col-6">
                                        ...
                                        foreach ($_SESSION['cart'] as $key => $value) {
                                            $count++;
                                            $num_items = $num_items+floatval($value['cantidad']);
                                            $value_pixel = $value_pixel+floatval($value['precio']);
                                            if ($content_ids=="") {
                                                $content_ids = "'".$value['producto_id']."'";
                                                $contents = "{'id': '".$value['producto_id']."', 'quantity':".$value['cantidad']."}";
                                            }else{
                                                $content_ids .= ",'".$value['producto_id']."'";
                                                $contents .= ",{'id': '".$value['producto_id']."', 'quantity':".$value['cantidad']."}";
                                            }
                                            if ($count==1) {
                                                ...
                                                $productos_id = $value['producto_id'];
                                                $productos_cantidad = $value['cantidad'];
                                                $productos_precio = $value['precio'];
                                                $productos_addons = $value['addons'];
                                            }else{
                                                    ...
                                                    if ($value['productos'][$i]['productos_valor_envio_nacional']==0) {
                                                        ...
                                                        if (isset($_SESSION['municipio_session']) && isset($_SESSION['departamento_session'])) {
                                                            ...
                                                            if ($p_ub==1) {
                                                                ...
                                                                $thisNacional = floatval($value['productos'][$i]['productos_valor_envio_nacional']);
                                                            }
                                                        }else{
                                                            $thisNacional = floatval($value['productos'][$i]['productos_valor_envio_nacional']);
                                                        }
                                        ...
                                        <div class="col-12 px-0">
                                            <label>
                                                <input checked type="radio" name="pedidos_metodo_pago" value="Transferencia">
                                                <strong>
                                        ...
                                        <div class="col-12 px-0">
                                            <label>
                                                <input type="radio" name="pedidos_metodo_pago" value="PayU Latam">
                                                <strong>
                                                    PAYU LATAM
                                                </strong>
                                            </label>
