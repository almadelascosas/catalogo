...
class Mailing_model extends CI_Model {
    ...
    public function mailPedidoRecibidoPrueba($pedido=array()){
        if ($pedido!=array()) {
            require_once($_SERVER['DOCUMENT_ROOT'].'/application/libraries/mailphp/config.php');
            $cuerpo = '
            <!DOCTYPE html>
            <html lang="en">
            <head>
            ...
            $mail->AddAddress("softmenaca@gmail.com");
    ...
    public function mailPedidoRecibido($pedido=array()){
        if ($pedido!=array()) {
            ...
            <body style="text-align:center;background-color:#fff7ec;padding-top:20px; padding-bottom:20px;width:100%;">
                <div style="background-color:#fff;width:100%;max-width:600px;padding-top:10px;padding-bottom:10px;margin:auto;">
                    ...
                    <table style="width:90%;margin-left:5%;margin-bottom:15px;border-collapse:collapse;">
                                            ...
                                            if ($value3['productos_id']==$value2['pedidos_detalle_producto']) {
                                                ...
                                                if ($agregado[$key2]!=1) {
                                                    ...
                                                    array_push($productosMostrar, array(
                                                        ...
                                                        'productos_cantidad' => floatval($value2['pedidos_detalle_producto_cantidad']),
                                                        'productos_vendedor' => $value3['productos_vendedor'],
                                                        'productos_fechaprogramada' => $value2['pedidos_detalle_fechaprogramada']
                                                    ));
                                                    $agregado[$key2] = 1;
                                                }
                                                if (count($productosMostrar)<=0) {
                        ...
                        $alertaFechaProgramada='';
                        foreach ($productosMostrar as $key4 => $value4) {
                            $txtFechaProgramada='';
                            if($value4['productos_fechaprogramada']!=='' && $value4['productos_fechaprogramada']!=='0000-00-00'){
                                date_default_timezone_set("America/Bogota");
                                setlocale(LC_TIME, "spanish");
                                setlocale(LC_ALL,"es_ES");
                                $newDate = date("d-m-Y", strtotime($value4['productos_fechaprogramada'])); 
                                $fechaMostrar = strftime("%A %d de %B %Y", strtotime($newDate));

                                $alertaFechaProgramada.='<br><b>Producto:</b> '.$value4['productos_titulo'].' - <b>Fecha Programada:</b> '.utf8_encode($fechaMostrar);

                                $txtFechaProgramada='<tr>
                                                        <td style="text-align:left;padding-bottom:15px;padding-top:15px;"><b>Fecha Programada:</b> </td>
                                                        <td style="text-align:left;padding-bottom:15px;padding-top:15px;">'.utf8_encode($fechaMostrar).'</td>
                                                    </tr>';
                            }
                            $cuerpo.='<tr><td style="text-align:left;padding-bottom:15px;padding-top:15px;">';
                            $this_price = 0;
                            $cuerpo.=$value4['productos_titulo']." $".number_format($value4['productos_precio'], 0, ',', '.')." x ".$value4['productos_cantidad']."<br>";
                            $this_price = $value4['productos_precio']*$value4['productos_cantidad'];
                            $cuerpo.='</td>
            ...
            require_once($_SERVER['DOCUMENT_ROOT'].'/application/libraries/mailphp/config.php');
            $mail->ClearAllRecipients( );
            $mail->AddAddress($pedido['pedido']['pedidos_correo']);
            $mail->IsHTML(true); 
            $mail->Subject = 'PEDIDO - ALMA DE LAS COSAS';
            ...
            if ($mail->Send()) {
                $msjEnvios['ok']+=1;
                $msjEnvios['correosok'][]=$pedido['pedido']['pedidos_correo'];
            } else {
                $msjEnvios['error']+=1;
                $msjEnvios['correoserror'][]=$pedido['pedido']['pedidos_correo'];
            }
            ...
            $this->db->where_in("usuarios_id",$ids_vendedores);
            ...
            if($alertaFechaProgramada!==''){
                $alertaFechaProgramada='<p>
                                    <b>!Pedido con Fechas Programadas.</b>
                                    <br>
                                    En este pedido, el usuario ha programado la entrega del(de los) producto(s) para una(s) fecha(s) proximas
                                    <br>
                                    '.$alertaFechaProgramada.'
                                    </p>';
            }
            for ($i=0; $i < count($correos); $i++) {
                ...
                <body style="text-align:center;background-color:#fff7ec;padding-top:20px; padding-bottom:20px;width:100%;">
                    <div style="background-color:#fff;width:100%;max-width:600px;padding-top:10px;padding-bottom:100px;margin:auto;">
                        ...
                        <div style="width:100%;margin-top:20px;margin-bottom:20px;">
                            ...
                            text-decoration: none;
                            box-shadow: 0px 3px 10px #ccc;
                            border-radius: 15px;color:#fff;font-weight:600;padding-left:40px;padding-right:40px;padding-top:10px;padding-bottom:10px;" href="'.base_url().'pedidos/editar_new/'.$pedido['pedido']['pedidos_id'].'">VER PEDIDO</a>
                        </div>
                ...
                if ($mail->Send()) {
                    $msjEnvios['ok']+=1;
                    $msjEnvios['correosok'][]=$correos[$i];
                } else {
                    $msjEnvios['error']+=1;
                    $msjEnvios['correoserror'][]=$correos[$i];
                }
            }
            ...
            for ($i=0; $i < count($correos); $i++) {
                ...
                <body style="text-align:center;background-color:#fff7ec;padding-top:20px; padding-bottom:20px;width:100%;">
                    <div style="background-color:#fff;width:100%;max-width:600px;padding-top:10px;padding-bottom:100px;margin:auto;">
                        ...
                        <div style="width:100%;text-align:center;">
                            <img style="width:100%;max-width:300px;margin:auto;" src="'.base_url('assets/img/mail-pedido-recibido.png').'" alt="Ilustracion">
                        </div>
                        <p style="font-weight:600;margin-top:15px;">Nuevo Pedido Recibido, Pedido nro. '.$pedido['pedido']['pedidos_id'].'</p>
                        <div style="width:100%;margin-top:20px;margin-bottom:20px;">
                            <a style="background-color: #e6a726;
                            ...
                            text-decoration: none;
                            box-shadow: 0px 3px 10px #ccc;
                            border-radius: 15px;color:#fff;font-weight:600;padding-left:40px;padding-right:40px;padding-top:10px;padding-bottom:10px;" href="'.base_url('pedidos/'.$pedido['pedido']['pedidos_id']).'">VER PEDIDO</a>
                        </div>
                        <h3>¡Hola tienes un nuevo pedido!</h3>
                        <p style="margin-bottom:15px;width:90%;margin-left:5%;">Ha llegado a tu bandeja de pedidos el siguiente pedido, te invitamos a que ingreses a tu perfil y consultes toda la información del pedido.</p>
                    </div>
...
'.$pedido['pedido']['pedidos_correo_envio'].'<pre></td>
            ...
            $mail->ClearAllRecipients( );
            $mail->AddAddress($pedido['pedido']['pedidos_correo']);
            //$mail->AddAddress('yepagu@gmail.com');
            $mail->IsHTML(true); 
            $mail->Subject = 'PEDIDO - ALMA DE LAS COSAS';
            ...
            if ($mail->Send()) {
                ...
                $datos['result']= 0;
                $datos['mensaje']= "Error al Enviar. ".$mail->ErrorInfo;
            }
            return $datos;
        }
    }
    ...
    public function mailUserDisabled($correo){
        $cuerpo = '
        <!DOCTYPE html>
        <html lang="en">
        ...
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>DATOS USUARIO ELIMINADOS - ALMA DE LAS COSAS</title>
        </head>
        ...
        <body style="text-align:center;background-color:#fff7ec;padding-top:20px; padding-bottom:20px;width:100%;">
            <div style="background-color:#fff;width:100%;max-width:600px;padding-top:10px;padding-bottom:100px;margin:auto;">
                ...
                <p style="font-weight:600;margin-top:15px;">
                    !Nombre Usuario... Lamentamos que nos abandones
                </p>
                <div style="width:100%;margin-top:20px;margin-bottom:20px;">
                    <a style="background-color: #e6a726;
                font-size: 12px;
                text-decoration: none;
                box-shadow: 0px 3px 10px #ccc;
                border-radius: 15px;color:#fff;font-weight:600;padding-left:40px;padding-right:40px;padding-top:10px;padding-bottom:10px;" href="https://almadelascosas.com">Ir a Alma de las Cosas</a>
                </div>
                <h3>¡Tus datos los hemos eliminado!</h3>
                <p style="margin-bottom:15px;width:90%;margin-left:5%;">Hemos eliminado todo dato tuyo de nuestra base de datos, solo los registros de compras estaran guardados, esto para fines de informes administrativos.</p>
            </div>
        </body>
        ...
        $mail->IsHTML(true); 
        $mail->Subject = 'DATOS USUARIO ELIMINADOS - ALMA DE LAS COSAS';
