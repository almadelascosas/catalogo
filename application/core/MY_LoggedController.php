...
class MY_LoggedController extends CI_Controller {
    ...
    public function __construct(){
          parent::__construct();
              // Check auth
        $this->_check_auth();
        $global_data = array('menu_app'=>$this->config->item('menu_app'));
        $this->load->vars($global_data);
        $this->load->vars(array('mostrar_paginacion'=>$this->config->item('mostrar_paginacion')));
        $this->load->vars(array('tipos_pagos'=>$this->config->item('tipos_pagos')));
        $CI =& get_instance();
    ...
    private function _check_auth(){
      $GLOBALS['accesoDisabled'] = "";
      if(!isset($_SESSION['usuarios_id'])){
        header("Location: /auth/login");
        exit;
      }else {
        $findme = $_SERVER["HTTP_HOST"]."/".$this->uri->segment(1);
        $this->db->select('*')->like("menu_dash_enlace",$findme)->limit(1);
        $menu_perm = $this->db->get('menu_dash');
        
        foreach ($menu_perm->result_array() as $key => $value) {
